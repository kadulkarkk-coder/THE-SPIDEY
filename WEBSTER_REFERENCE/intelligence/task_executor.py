...
class TaskExecutor:
    ...
    def __init__(self, planner: PlanningEngine, router: ActionRouter, progress: ProgressReporter) -> None:
    ...
    def execute(self, goal: str, *, task_id: str | None = None) -> TaskExecutionResult:
        ...
        if not plan.steps:
            ...
            return TaskExecutionResult(task_id, False, plan, (), "empty plan")
        ...
        for step in plan.steps:
            ...
            if step not in ready:
                ...
                return TaskExecutionResult(task_id, False, plan, tuple(results), "dependency not satisfied")
            ...
            if not action.handled or not action.ok:
                ...
                return TaskExecutionResult(task_id, False, plan, tuple(results), action.message or "step failed")
        ...
        return TaskExecutionResult(task_id, True, plan, tuple(results))
